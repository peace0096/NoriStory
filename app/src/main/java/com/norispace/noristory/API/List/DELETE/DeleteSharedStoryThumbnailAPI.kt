...
object DeleteSharedStoryThumbnailAPI {
    fun call(model: SharedStoryThumbnail_Model, callback: RetrofitClient.callback) {
        val body = JSONObject()
        try {
        ...
        RetrofitClient.getBaseClient().deleteSharedStoryThumbnail(User_Repo.getToken(), body.toString()).enqueue(object:
            Callback<String?> {
